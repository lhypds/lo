// The four ways off lo, in one list read from both ends: the sheet that fills
...
//
// All four are shown wherever they are filled in. Which of them a person is
...
// be asked for all three.
//
// `link` is for the handles that are an address in their own right — an email
// one, and a phone number written the way WhatsApp's own links need it. A LINE
// ID and a WeChat ID are neither: they are typed into that app by hand, so those
// rows are text to be read off and copied, which is all lo can honestly make of
// them.
...
export function contactsFor(language, filled = {}) {
  ...
  return CONTACTS.filter(
    (contact) =>
      contact.field === "email" || contact.field === messenger || Boolean(filled[contact.field]),
  );
}

// The profile as the API takes it: all five fields, whatever is in them. What is
...
//
// All five even when the sheet only asked for two: a WeChat ID entered in Chinese
// is still that account's WeChat ID when the same person comes back in English,
// and a form that sent only what it drew would take it down for them.
